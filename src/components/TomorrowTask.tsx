        ...
        <section className="w-full flex flex-col gap-20 pb-10">
            ...
            <div className="flex flex-col gap-12">
                {filteredTasks.map((task) => (
                    <Task key={task.id} getTasks={getTomorrowTasks} title={task.title} task={task} completedTask={completedTask} isCompletedTask={task.completed} onClick={() => router.push(`/tasks/${task.id}`)} isOverdueTask={!!(!task.completed && task.dueDate && new Date(task.dueDate).setHours(0, 0, 0, 0) < new Date().setHours(0, 0, 0, 0))} />
                ))}
            </div>
        </section>    
    )
